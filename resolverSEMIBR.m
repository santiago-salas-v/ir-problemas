...
if Isot
    ...
    odeOptions=odeset('Mass',M,'OutputFcn',@odeprog,...
        'Events',@odeabort);    
    %Resolver IVP para sistema de EDO en C.
    ...
    Ta_t0 = fsolve(@(Ta0_t0Var)...
        ...
        U*A./(Vr0*Cp_Molares*C_t0').*(Ta0_t0Var-T_t0),...
        ...
    ...
    Ta0_t0 = fsolve(@(Ta0_t0Var)...
        ...
        rapideces(C(:,1),...
        T_t0,Exponentes_r,k0,E,R,T0ref))+...
        U*A/((Vr0+Q0*t(1))*Cp_Molares*C(:,1))*...
        (Ta_t0-Ta0_t0Var)/...
        log((Ta_t0-T_t0)/(Ta0_t0Var-T_t0))+...
        Q0/(Vr0+Q0*t(1))*...
        (Cp_Molares*C0'/(Cp_Molares*C(:,1)))*...
        (T0-T_t0),...
        ...
        Ta_t0+1e-10,optimset('Display','off','TolX',1e-4^2));
    %Resolver IVP para sistema de EDO en Ta(t) y Ta0(t)
    %Considerar forma M*y'=f(t,y), especificar f(t,y) como
    %@(t,y), sistema diferencial - algebraico: M=[1,0;0,0]
    M           =eye(2);
    M(end,end)  =0;
    odeOptions  =odeset('Mass',M,'OutputFcn',@odeprog,...
        'Events',@odeabort);
    sol_Ta_y_Ta0=ode15s(@(t,y)...
        [...
        -U*A/(Va*rhoCp_a)*...
        (y(1)-y(2))/log((y(1)-T_t0)/(y(2)-T_t0))+...
        Qa0/Va*(y(2)-y(1));...
        ...
        (1/(Cp_Molares*deval(sol_C,t)))*...
        (-delta_Hr(T_t0,delta_Hf,Coefs_esteq,Cp_Molares)*...
        rapideces(deval(sol_C,t),...
        T_t0,Exponentes_r,k0,E,R,T0ref))+...
        U*A/((Vr0+Q0*t)*Cp_Molares*deval(sol_C,t))*...
        (y(1)-y(2))/log((y(1)-T_t0)/(y(2)-T_t0))+...
        Q0/(Vr0+Q0*t)*...
        (Cp_Molares*C0'/(Cp_Molares*deval(sol_C,t)))*...
        (T0-T_t0)...
        ]...
        ,[t(1),t(end)],[Ta_t0,Ta0_t0],odeOptions);
    [y,~]=deval(sol_Ta_y_Ta0,t);
    Ta  =y(1,:);
    Ta0 =y(2,:);
    sol={sol_C,sol_Ta_y_Ta0};
elseif ~Isot    
