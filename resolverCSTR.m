...
elseif ~Estacionario
    if Isot
        ...
        if Incompresible
            ...
            Ta=fsolve(@(TaVar)...
                ...
                Q0/Vr*(Cp_Molares*C0'./(Cp_Molares*C))*(T0-T_t0)+...
                (1./(Cp_Molares*C)).*(...
                (-delta_Hr(T_t0,delta_Hf,Coefs_esteq,Cp_Molares))*...
                rapideces(C,T_t0,Exponentes_r,k0,E,R,T0ref)...
                )+...
                U*A./(Vr*Cp_Molares*C).*...
                (TaVar-Ta0)./log((TaVar-T_t0)./(Ta0-T_t0))...
                ...
                ,...
                (T_t0-(T_t0-Ta0)*exp(-U*A./(Qa0*rhoCp_a)))*...
                ones(size(t)),optimset('Display','off'));
            dTa_en_dt=...
                interp1(...
                cumsum([0,diff(t(1:end-1))/2]+[diff(t(2:end))/2,0])...
                ,diff(Ta)./diff(t),t,'linear','extrap');
            Qa0=Va./(Ta0-Ta).*(...
                dTa_en_dt+...
                U*A./(Vr*Cp_Molares*C).*...
                (Ta-Ta0)./log((Ta-T_t0)./(Ta0-T_t0))...
                );
    ...
    elseif ~Isot
        if Incompresible
            ...
            sol=ode15s(@(t,y)...
                ...
                ...
                Q0/Vr*(Cp_Molares*C0'./(Cp_Molares*y(1:end-2))).*...
                (T0-y(end-1))+...
    ...
    qgen=1./(rho_Cp).*(sum(...
        (-delta_Hr(T,delta_Hf,Coefs_esteq,Cp_Molares))'.*r...
        ,1));
    qrem=-Q0/Vr*(Cp_Molares*C0'./(Cp_Molares*C)).*...
        (T0-T)+...
        -(Qa0*rhoCp_a)./(Vr*rho_Cp).*(Ta0-T).*(1-exp(-U*A./(Qa0*rhoCp_a)));
    if isscalar(Qa0)
        Qa=Qa0*ones(size(T));
